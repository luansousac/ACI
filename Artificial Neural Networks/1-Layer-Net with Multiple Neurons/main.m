...
fprintf('|               REDES DE UMA CAMADA MULTI-NEURÔNIOS              |\n');
...
%% Training the network
    ...
    [X_training, Y_training, X_test, Y_test] = shuffle_data(norm_data, n_neurons);

    % Initiating random weights for each of the neurons of the output layer
    W = init_weights(norm_data, n_neurons);
...
%% Ploting results for the report
    ...
    if opt == 4
        ...
        plot(D1(:,1), D1(:,2), '*', 'Color', [0.60 0.20 0.00], 'DisplayName', 'Classe 1');
        plot(D2(:,1), D2(:,2), '*', 'Color', [0.00 0.45 0.74], 'DisplayName', 'Classe 2');
        plot(D3(:,1), D3(:,2), '*', 'Color', [0.31 0.31 0.31], 'DisplayName', 'Classe 3');
...
fprintf('\nSummarizing, the 1-layer with muitple neurons was tested with...\n');
